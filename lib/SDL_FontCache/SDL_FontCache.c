...
static void FC_DrawColumnFromBufferSingleLine(FC_Font *font, FC_Target *dest, FC_Rect box, int *total_height, FC_Scale scale, FC_AlignEnum align) {
  int y = box.y;
  FC_StringList *ls, *iter;

  ls = FC_Explode(fc_buffer, '\n');
  for (iter = ls; iter != NULL; iter = iter->next) {
    FC_RenderAlign(font, dest, box.x, y, box.w, scale, align, iter->value);
//    y += FC_GetLineHeight(font);
  }
...
FC_Rect FC_DrawBoxContainedLine(FC_Font *font, FC_Target *dest, FC_Rect box, const char *formatted_text, ...) {
  Uint8 useClip;
  if (formatted_text == NULL || font == NULL)
  ...
  FC_DrawColumnFromBufferSingleLine(font, dest, box, NULL, FC_MakeScale(1, 1), FC_ALIGN_LEFT);
...
FC_Rect FC_DrawScale(FC_Font *font, FC_Target *dest, float x, float y, FC_Scale scale, SDL_Color color, const char *formatted_text, ...) {
  if (formatted_text == NULL || font == NULL)
    return FC_MakeRect(x, y, 0, 0);
...
FC_Rect FC_DrawScaleRightColor(FC_Font *font, FC_Target *dest, float x, float y, FC_Scale scale, SDL_Color color, const char *formatted_text, ...) {
  if (formatted_text == NULL || font == NULL)
    return FC_MakeRect(x, y, 0, 0);
...
FC_Rect FC_DrawScaleLeftColor(FC_Font *font, FC_Target *dest, float x, float y, FC_Scale scale, SDL_Color color, const char *formatted_text, ...) {
  if (formatted_text == NULL || font == NULL)
    return FC_MakeRect(x, y, 0, 0);
...
FC_Rect FC_DrawScale_Center(FC_Font *font, FC_Target *dest, float x, float y, FC_Scale scale, SDL_Color color, const char *formatted_text, ...) {
  if (formatted_text == NULL || font == NULL)
    return FC_MakeRect(x, y, 0, 0);
  ...
  return FC_RenderCenter(font, dest, x, y, scale, fc_buffer);
}
